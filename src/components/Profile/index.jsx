...
const ProfileComponent = ({theme}) => {
  ...
  return (
    <div className='flex flex-col justify-center items-center'>
      ...
      <p className={`font-light text-center text-xs leading-4 w-[360px] text-txt-dark`}>{profile.description}</p>
      <div className='flex gap-6 justify-around m-6'>
        <Button classname='bg-[#24272C] btn' txt={profile.btn1.value} link={profile.btn1.link}/>
        <Button classname='bg-[#24272C] btn' txt={profile.btn2.value} link={profile.btn1.link}/>
      </div>
    </div>
  )
}
